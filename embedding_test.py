...
def test_embedding_that_would_be_unsolvable_not_possible():
    """Tests that a block cannot be embedded into a node that could not
    relay its information to the sink"""
    infra = InfrastructureNetwork()

    nso = infra.add_source(pos=(0, 0), transmit_power_dbm=30, name="nso1")
    nin1 = infra.add_intermediate(
        pos=(1, 0), transmit_power_dbm=30, name="nin1"
    )
    # ridiculous transmit power to make sure any data rate can be
    # supported
    nin2 = infra.add_intermediate(
        pos=(1, 0), transmit_power_dbm=300, name="nin2"
    )
    _nsi = infra.set_sink(pos=(2, 0), transmit_power_dbm=30, name="nsi")
    ...
    overlay.add_link(bso, bin_, datarate=1)
    overlay.add_link(bin_, bsi, datarate=100)

    embedding = PartialEmbedding(infra, overlay, source_mapping=[(bso, nso)])

    eso = ENode(bso, nso)
    ein1 = ENode(bin_, nin1)
    ein2 = ENode(bin_, nin2)

    assert embedding.is_solvable()  # by embedding bin into nin2

    # This would embed bin into nin1. But nin1 does not have sufficient
    # transmit power to support the outlink from bin, so this would make
    # it impossible to finish the solution. Make sure that option is not
    # available.
    assert not embedding.take_action(eso, ein1, 0)

    # Now take the sensible embedding
    assert embedding.take_action(eso, ein2, 0)

    # Using nin1 as a relay would also make it impossible to finish the
    # solution
    assert not embedding.take_action(ein2, ENode(None, nin1), 1)


def test_impossible_relay_not_choosable():
    """Regression test for relays for source nodes with insufficient
    transmit power"""
    infra = InfrastructureNetwork()

    N3 = infra.add_source(
        name="N3", pos=(3.09, 6.31), transmit_power_dbm=19.02
    )
    N2 = infra.add_source(name="N2", pos=(8.89, 9.79), transmit_power_dbm=7.96)
    _N1 = infra.set_sink(name="N1", pos=(2.89, 4.98), transmit_power_dbm=14.16)

    overlay = OverlayNetwork()
    B3 = overlay.add_source(name="B3", requirement=0.001)
    B2 = overlay.add_source(name="B2", requirement=6.26)
    B1 = overlay.set_sink(name="B1", requirement=1.61)
    overlay.add_link(B2, B1, 6.59)
    overlay.add_link(B3, B1, 21.11)

    embedding = PartialEmbedding(
        infra, overlay, source_mapping=[(B2, N2), (B3, N3)]
    )

    # n2 cannot support the outlink of b3
    assert not embedding.take_action(ENode(B3, N3), ENode(None, N2), 0)
